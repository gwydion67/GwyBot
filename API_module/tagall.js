...
export async function tagAll(m, sock) {
  ...
  try {
    ...
    const groupMetadata = chatJid.endsWith("@g.us")
      ? await sock.groupMetadata(chatJid).catch((e) => {
          console.log("cant get metadata ", e);
        })
      : "";
    const participants = chatJid.endsWith("@g.us")
      ? await groupMetadata.participants
      : "";
    const groupAdmins = chatJid.endsWith("@g.us")
      ? getGroupAdmins(participants)
      : "";
    const isAdmins = chatJid.endsWith("@g.us")
