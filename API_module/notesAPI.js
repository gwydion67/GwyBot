...
export const getNotes = async (Note, chatJid, count) => {
  let msg = "";
  await Note.find({ inChat: chatJid })
    .exec()
    .then((data) => {
      console.log("here");
...
export const removeNote = async (Note, chatJid, index) => {
  ...
  for await (let i of index) {
    await Note.deleteOne({ inChat: chatJid, index: parseInt(i) })
      .then((data) => {
        res = true;
      })
      .catch((err) => {
...
export const handleAddNote = async (m, Note, sock) => {
  ...
  sock.sendMessage(chatJid, {
    text: res ? "note created" : "note creation failed",
  });
};
...
export const handleDeleteNotes = async (m, Note, sock) => {
  ...
  sock.sendMessage(chatJid, {
    text: res ? "notes deleted" : "notes deletion failed",
  });
};
